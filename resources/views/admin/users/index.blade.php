            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($users as $user)
                    <tr>
                        ...
                        <td>{{ $user->email }}</td>
                        <td>{{ $user->phone }}</td>
                        <td>{{ $user->status == 'allow' ? 'تمكين' : 'تعطيل' }}</td>

                        <td class="text-center">
                            <a href="{{ route('admin.users.show', $user->id) }}" class="btn btn-primary"><i class="fa fa-eye"></i></a>
                        </td>
                        <td class="text-center">
                            <a href="{{ route('admin.users.edit', $user->id) }}" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                        </td>
                        <td class="text-center">
                            <a href="{{ route('admin.users.delete',encrypt( $user->id)) }}" onclick="return confirm('هل أنت متأكد من الحذف');" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                        </td>
