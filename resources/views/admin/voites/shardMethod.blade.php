...
@section('js')
    ...
    <script>
        ...
        function deleteOption(id, encId) {
            $('#option-row' + id).remove();
            var newInput = '<input type="hidden" name="optionDeletedId[]" value="' + encId + '">';
        ...
        function addOption() {
            ...
            html += '<input type="hidden" value=""  name="option_value[' + option_row + '][id]" >';

            html += '  <td class="text-right"><input type="text"  name="option_value[' + option_row + '][name]" " placeholder="" class="form-control" /></td>';
            html += '  <td class="text-left"><button type="button" onclick="$(\'#option-row' + option_row + '\').remove();" data-toggle="tooltip" title="" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
            html += '</tr>';
